...
async def auto_save_anime(message: types.Message):
    if message.chat.id == CHANNEL_ID:
        ...
        # Animeni bazadan qidiramiz, yo'q bo'lsa yangi ochamiz
        ...
        # Qism bazada bormi tekshiramiz, bo'lmasa qo'shamiz
        cursor.execute(
...
# 3. Foydalanuvchi botga yozganda (Anime nomi yoki raqam yozganda)
...
async def search_anime(message: types.Message):
    ...
    if text.isdigit():
        target_ep = int(text)
        # Shu qism raqami bazada borligini tekshiramiz
        cursor.execute("""
        ...
        if results and len(results) == 1:
            ...
            conn.close()
            await message.answer("❌ Bu raqamdagi qism topilmadi. Iltimos, avval anime nomini to'liq yuboring.")
            return
    ...
    if not anime:
        conn.close()
        await message.answer("❌ Kechirasiz, bu nomdagi anime topilmadi yoki hali qismlari yuklanmagan.")
        return
